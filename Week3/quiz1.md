...
- 답: 
* Merge Commit는 두 브랜치를 공통 부모로 하는 새로운 commit을 만든다.
* Squash and Merge는 하나의 커밋으로 main 브랜치로 병합된다.
* Rebase and Merge는 다른 커밋의 base를 재설정한 후 모두 새로운 커밋으로 변경한다.
...
- 답: commit id: a1s2d3f. develop브랜치에서 작업중, 최근 커밋 : docs: readme 추가
...
- 답: git refert a1s2d3f

### Q9
여러 사람이 협업하는 프로젝트에서 커밋을 되돌려야 한다.  
reset과 revert 중에 어떤 것을 선택할 것인지를 그 이유와 함께 적으시오.

- 답: revert를 사용할 것 같다. reset은 commit을 삭제하는데 commit을 공유하는 다른 브랜치에도 영향을 줄 수 있기에 생성하여 되돌리는 revert가 더욱 안전하기 때문이다.
